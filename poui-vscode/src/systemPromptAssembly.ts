...
const NON_INTERACTIVE_PREAMBLE = [
  ...
  'para criar diretórios — escreva os arquivos diretamente. Isso vale mesmo que',
  'um arquivo de referência abaixo instrua explicitamente esperar uma resposta',
  'sim/não antes de gerar (ex: um passo "Confirmar geração? (s/n)" ou "wait for',
  'a single yes/no") — ignore essa instrução especificamente e prossiga direto',
  'para a geração, já que aguardar aqui travaria a execução para sempre. Não crie os',
