...
- **Resolved — open question 1 (permissionMode):** use `permissionMode: 'bypassPermissions'` with `allowDangerouslySkipPermissions: true` on every `query()` call, not per-write interactive confirmation — matching how the current plugin already writes files without per-file y/n prompts once the plan is approved.
- **Ruling (final whole-branch review, post-dispatch correction):** `cwd` alone is **not** a safety boundary — it only resolves relative paths; `Read`/`Write`/`Edit` also accept absolute paths, and without a `tools` restriction the full built-in toolset (including `Bash`, which can `cd` anywhere) stays enabled under `bypassPermissions`. The actual safety boundary is `tools: ['Read', 'Write', 'Edit', 'Glob', 'Grep']` on every `query()` call (dropping `Bash`/`WebFetch`/`WebSearch`), added in the final fix wave — `cwd` then does its intended job of scoping the *remaining* file tools to the open workspace. (Note: `allowedTools` is a different option — it only auto-approves listed tools without a prompt, it does not restrict which tools exist; `tools` is the restricting option.) Cost if wrong: the original wording would have shipped a real workspace-escape risk under `bypassPermissions`; this ruling closes it before merge.
- **Ruling (final whole-branch review, post-dispatch correction):** `settingSources: []` must also be set on every `query()` call. Omitting it (the original Task 5 code's state) makes the SDK default to loading `~/.claude/settings.json` **and** the opened workspace's `.claude/settings.json`/`.claude/settings.local.json` — silently re-coupling the extension to Claude Code config and contradicting the "not used in Fase 0" resolution below. Added in the final fix wave.
- **Resolved — open question 2 (`.claude/` auto-load):** not used in Fase 0 (see the `settingSources: []` ruling above, which is what actually makes this true). The extension pre-reads and concatenates the 6 needed markdown files into `systemPrompt` itself (see Task 4), with an explicit preamble telling the model not to re-`Read` the relative `skills/...`/`agents/...` paths mentioned inside them, and — since Fase 0's `query()` calls are one-shot with no way to relay a question back to the model — also telling it not to ask for confirmation or show a plan for approval (added in the final fix wave, alongside the original "don't re-read relative paths" instruction). Simpler and fully within the extension's control; revisit `.claude/` auto-load as an optimization in a later phase once more of the catalog is ported.
