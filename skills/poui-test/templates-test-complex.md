...
- **`provideHttpClient()` obrigatório mesmo sem HTTP no service** (two-panel-browse / qualquer componente PO-UI): módulos PO-UI internamente requerem `HttpClient` para ícones e recursos. Mesmo que o service do componente use apenas `of(...)` sem HTTP, omitir `provideHttpClient()` gera `NullInjectorError: No provider for HttpClient!`. Sempre incluir `provideHttpClient()` + `provideHttpClientTesting()` nos providers do TestBed. Manter `afterEach(() => httpMock.verify())` — passará sem erros pois não há requests pendentes.
